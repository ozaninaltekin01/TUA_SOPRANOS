...
# ── 18-ozellikli mock egitim verisi ──────────────────────────────────────────
# Her satir bir conjunction ornegi.
# Kolonlar: ml_model.py FEATURE_NAMES sirasina gore.
# Etiketler: 0=RED, 1=YELLOW, 2=GREEN

X = np.array([
    # RED — cok yakin, yuksek hiz
...
# ── Kaydet — ml_model.py'nin beklettigi dict formati ─────────────────────────
model_data = {
    "model":         model,
    "accuracy":      accuracy,
...
out_path = "model/xgboost_risk_model.pkl"
joblib.dump(model_data, out_path)
print(f"Saved → {out_path}  (joblib, current env versions)")
